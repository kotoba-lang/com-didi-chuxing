(ns didi_chuxing.main
  "Kotodama WASM entrypoint for the Didi_chuxing clean-room actor (L4) — Clojure port.
  ...
  facts (`didi_chuxing.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "didi_chuxing")
(def tier "L4")
...
(def entity-specs
  [{:entity "Driver"  :plural "drivers"  :id-prefix "didichux_dri"
   ...
   {:entity "Rating"  :plural "ratings"  :id-prefix "didichux_rat"
...
(defn emit-facts
  "EAVT facts for one record: {\"didi_chuxing.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "didi_chuxing-compat" :tier tier :entities entities} 200])
...
(defn start! [] :didi_chuxing-compat/ready)
